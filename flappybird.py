...
def main():
    ...
    while not exit:
        ...
        while not done:
            ...
            pygame.display.flip()
            if not isclimbing:
                curstate = stategen(bird, pipes[0], isdead)
                if frame_clock > 0 and prevstate is not None and lastaction is not None:
                    orthQ(curstate, prevstate, topscore)
                prevstate = deepcopy(curstate)
                if rounds < 30 and topscore <= 7 and not fileexists:
                    explore = True
                else:
                    explore = False
                actionchooser(curstate, explore)
                takeaction(bird)
            elif isdead:
                curstate = stategen(bird, pipes[0], isdead)
                orthQ(curstate, prevstate, topscore)
